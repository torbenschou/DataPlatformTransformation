...
/*
  ...
  NB: Must be enabled on instance
...
--
-- HOME PIE CHARTS REPORT
--

EXEC  [SqlServer_Reporting].[UIPieChartSqlServerProducts]


DECLARE @myEditions TABLE 
(
  TotalPaid INT,
  Enterprise INT,
  EnterpriseCore  INT,
  ...
  Desktop INT,
  Developer INT,
  TopReportedEditions INT,
  OtherReportedEditions INT
)

INSERT INTO @myEditions EXEC [SqlServer_Reporting].[UISqlServerEditions]

SELECT Edition, Numbers
FROM 
  (SELECT Enterprise, EnterpriseCore, DataCenter,Standard,Web,BusinessIntelligence,Workgroup,Unknown,Evaluation,Express,Desktop,Developer FROM @myEditions) p
UNPIVOT 
([numbers] FOR [Edition] IN ( Enterprise, EnterpriseCore, DataCenter,Standard,Web,BusinessIntelligence,Workgroup,Unknown,Evaluation,Express,Desktop,Developer)) as unP
WHERE Numbers > 0;

EXEC [SqlServer_Reporting].[UIPieChartSqlServerComponents]

EXEC [SqlServer_Reporting].[UIPieChartSqlServerLicensedEditions]
...
------
    ...
    SELECT hic.[ComputerName]																		 AS 'Computer Name',
            ...
            FROM   [Core_Inventory].[Devices] d
             ...
             INNER JOIN [AllDevices_Assessment].[HardwareInventoryCoreView] hicv 
               ON d.[DeviceNumber] = hicv.[DeviceNumber]
			   where hic.[ComputerName] NOT like '%IBM%'
      ORDER  BY 1
 


------
------ OS Information of servers hosting SQL Server components
------
...
--  --AND [SQL Server Edition] NOT in ('Express', 'Developer')
--  --AND [SQL Server Edition] in ('Standard', 'Unknown', 'Desktop Engine')
--  AND [SQL Server Edition]  in ('Express', 'Developer')
--    AND [Computer Name] NOT IN (SELECT [Computer Name]
--  FROM [ICA_05042017].[dbo].[ICA_ConsolidationServer]
--  where [SQL Server Edition] NOT IN  ('Express','Developer'))
--  group by 
--  [Computer Name]
--      ,[Machine Type]
--      ,[Operated By]
